...
View<double*,double,3> evaluator(std::map<std::string, double*> bigVectors){
	...
	deviceQueue.submit([&] (cl::sycl::handler &cgh) {
		...
		ParMap([=](double x){return
		[=](View<accessor,double> result, unsigned thread_id){
		result=3.0*x;};},a162533727,v_898856380,8);
	});
	deviceQueue.submit([&] (cl::sycl::handler &cgh) {
		act_cgh = &cgh;
		View<accessor,double,3>a162533727(std::array<size_t,1>{1},b_a162533727.get_access<rw_access>(cgh));
		...
		[=](double y){return
		[=](View<accessor,double> result, unsigned thread_id){
		result=x+y;};};},v_898856380,b931609811,v_2147482884,8);
	});
	return result;
}
