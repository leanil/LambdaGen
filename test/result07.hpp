...
View<double*,double,2,2> evaluator(std::map<std::string, double*> bigVectors){
	...
	deviceQueue.submit([&] (cl::sycl::handler &cgh) {
		...
		View<accessor,double,3> v_203958725(std::array<size_t,1>{1},b_203958725.get_access<rw_access>(cgh));
		act_cgh->single_task<class SingleKernel>([=] () mutable {
			...
			[=](double y){return
			[=](View<accessor,double> result){
			result=x*y;};};},v1,v2,v_203958725),
			Reduce([=](double x){return
			[=](double y){return
			[=](View<accessor,double> result){
			result=x+y;};};},v_203958725,result);};};}(v),m2,result);};},m1,result);};};}(mat844138389)(mat1838073155)(v_2147482884);
		});
	});
	return result;
}
